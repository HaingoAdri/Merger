<?php
	...
	function setChampionOuest1( $pdo , $value , $type , $details , $index){
		$select = getFromBlockOuest( $pdo , $type );
		if( $type == 1 ) return;
		for( $i = 0 ; $i < count($select)-1; $i+=2 ){
			$score1 = randomScore();
			...
			if( $score1 < $score2 ){
				...
				$idPerdant = $select[rand($i,$i+1)]['idEquipe'];
			}
			$insert = "insert into blockouestP values( '%s' , '%s' , %d)";
			$insert = sprintf($insert , $idPerdant , $details[$index] , $type / 2);
			$pdo->exec( $insert );
		}
		setChampionOuest1( $pdo , $value , $type / 2 , $details,$index + 1 );
	}
	...
	function setChampionEst1( $pdo , $value , $type , $details , $index){
		$select = getFromBlockEst( $pdo , $type );
		if( $type == 1 ) return;
		for( $i = 0 ; $i < count($select)-1; $i+=2 ){
			$score1 = randomScore();
			...
			$insert = "insert into blockestP values( '%s' , '%s',%d)";
			$insert = sprintf($insert , $idPerdant , $details[$index] , $type / 2);
			$pdo->exec( $insert );
		}
		setChampionEst1( $pdo , $value , $type / 2 , $details ,$index + 1 );
	}
	...
	function get2EstChampion( $pdo ){
		$select = "Select * from blockestP as b join equipe on b.idEquipe = equipe.idEquipe join pool on pool.idPool = equipe.idPool where types = 1";
		$select = $pdo->query($select);
		$valiny	= $select->fetchAll(PDO::FETCH_ASSOC);
		$valiny = $valiny[0];
		return $valiny;
	...
	function get2OuestChampion( $pdo ){
		$select = "Select * from blockouestP as b join equipe on b.idEquipe = equipe.idEquipe join pool on pool.idPool = equipe.idPool where types = 1";
		$select = $pdo->query($select);
		$valiny	= $select->fetchAll(PDO::FETCH_ASSOC);
		$valiny = $valiny[0];
		return $valiny;
	...
	function get3eme($pdo){
		$query ="Select * from 3emefinals";
		$query = $pdo->query($query);
		$valiny = $query->fetchAll(PDO::FETCH_ASSOC);
		$valiny = $valiny;
	...
	function insert3emeFinalMatch($pdo , $idE1 , $idE2 , $score){
		$insert = "insert into 3emefinalMatch values ( '%s' , '%s' , '%s' )";
		$insert = sprintf($insert , $idE1 , $idE2 , $score);
		$pdo->exec( $insert );
	}
